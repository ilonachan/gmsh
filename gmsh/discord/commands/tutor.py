...
import gmsh.database as db
from gmsh.discord import discord_handler
from gmsh.discord.commands import gmsh_command, CmdUsage, CommandContext, codify
...
async def send_react_msg(ctx, subject):
    ...
    return db.ReactionMessage(id=message.id, subject=subject)
...
async def handle_react_msg_pos(client: Client, payload: RawReactionActionEvent):
    ...
    try:
        subject = session.query(db.Subject).join(db.ReactionMessage)\
            .filter(db.ReactionMessage.id == payload.message_id).one()
