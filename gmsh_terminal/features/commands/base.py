...
from gmsh_terminal.features import dpyserver
from gmsh_terminal.features.commands import CmdUsage, gmsh_command, codify, CommandContext
...
async def invite_command(ctx: CommandContext, args, metadata=None):
    await ctx.channel.send(discord.utils.oauth_url(ctx.client.user.id, Permissions(dpyserver.bot_cfg['permissions'])))
